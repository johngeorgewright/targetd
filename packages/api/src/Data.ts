import type TargetingPredicates from './parsers/TargetingPredicates.js'
import {
  objectEntries,
  objectEveryAsync,
  objectFitler,
...
import { type DataItemsOut, DataItemsParser } from './parsers/DataItems.js'
import type { DataItemIn, DataItemOut } from './parsers/DataItem.js'
import type { DataItemRule } from './parsers/DataItemRule.js'
import type { MaybeArray, MaybePromise, ZodPartialObject } from './types.js'
...
import { PromisedData } from './PromisedData.js'
import { resolveVariables } from './parsers/DataItemVariableResolver.js'
...
export default class Data<$ extends DT.Meta> implements IData<$> {
  ...
  private constructor(
    ...
    this.#fallThroughTargetingParsers = Object.freeze(fallThroughTargetingParsers)
  ...
  async usePayload<Parsers extends $ZodShape>(
    parsers: Parsers,
  ): Promise<Data<$ & { PayloadParsers: $['PayloadParsers'] & Parsers }>> {
  ...
  #mergePayloadParsers<Parsers extends $ZodShape>(parsers: Parsers): $['PayloadParsers'] & Parsers {
    return {
      ...this.#payloadParsers,
      ...parsers,
    }
  ...
  > {
    ...
    const targetingParsers: $$['TargetingParsers'] = this.#mergeTargetingParsers(targeting)

    const targetingPredicates = this.#mergeTargetingPredicates(targeting) as TargetingPredicates<$$>

    const queryParsers: $$['QueryParsers'] = this.#mergeQueryPredicates(targeting)
  ...
  #mergeTargetingPredicates(targeting: TT.DescriptorRecord) {
    return {
      ...
      ...objectMap(targeting, (targetingDescriptor) => ({
        ...
        requiresQuery:
          'requiresQuery' in targetingDescriptor ? targetingDescriptor.requiresQuery : true,
      })),
    }
  }
  ...
  > {
    ...
    const fallThroughTargetingParsers = this.#mergeFallThroughTargeting(targeting)
  ...
  async getPayload<Name extends keyof $['PayloadParsers']>(
    ...
    let payload: PT.Payload<$, $['PayloadParsers'][Name]> | undefined
    ...
    const nonResolvableVariables = objectFitler(variables, this.#isFallThroughRulesPayload)
    const resolvedPayload = resolveVariables(resolvableVariables, payload)

    return objectSize(nonResolvableVariables)
      ? {
          __variables__: objectMap(nonResolvableVariables, (value) => value.__rules__),
  ...
  async #getVariables<Name extends keyof $['PayloadParsers']>(
    ...
    predicate: (rule: DataItemRule<$, $['PayloadParsers'][Name]>) => Promise<boolean>,
  ) {
    const variables: Record<string, any> = {}
    if (objectSize(targetableItem.variables)) {
      for (const [variableName, rules] of objectEntries(targetableItem.variables)) {
  ...
  async #createRulePredicate<Name extends keyof $['PayloadParsers']>(
    ...
    return (rule: DataItemRule<$, $['PayloadParsers'][Name]>) =>
      (!('targeting' in rule) ||
        this.#targetingPredicate(
          query as any,
          rule.targeting! as any,
          objectMap(this.#targetingPredicates, (target, targetingKey) => ({
            predicate: () =>
              target.predicate(
  ...
  async #targetingPredicate(
    ...
    for (const targeting of targetings) {
      if (
        await objectEveryAsync(targeting, async (targetingValue, targetingKey) => {
