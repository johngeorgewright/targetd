...
export function attachVariableResolver<Parser extends $ZodType>(
  ...
  switch (parser._zod.def.type) {
    ...
    default:
      return variableResolverParser(variablesRegistry, parser) as RecursiveVariableResolver<Parser>
  }
}

function variableResolverParser<Parser extends $ZodType>(
  variablesRegistry: VariablesRegistry,
  parser: Parser,
): WithVariableResolver<Parser> {
  return (
    parser._zod.def.type === 'string'
      ? pipe(
          parser,
          transform((input, ctx) =>
            DataItemVariableResolverTransformer(variablesRegistry, parser, input as string, ctx),
          ),
        )
      : zodSwitch([
          [variableStringParser(), DataItemVariableResolverParser(variablesRegistry, parser)],
          [any(), parser],
        ])
...
function arrayVariableResolverParser<Parser extends $ZodArray>(
  ...
  const $arrayParser = new arrayParser._zod.constr({
    ...
    element: attachVariableResolver(variablesRegistry, arrayParser._zod.def.element),
  })
...
function recordVariableResolverParser<Parser extends $ZodRecord>(
  ...
  const $recordParser = new recordParser._zod.constr({
    ...
    valueType: attachVariableResolver(variablesRegistry, recordParser._zod.def.valueType),
  })
  return variableResolverParser(
    variablesRegistry,
    $recordParser,
...
function objectVariableResolverParser<Parser extends ZodObject | ZodMiniObject>(
  variablesRegistry: VariablesRegistry,
  objectParser: Parser,
): RecursiveVariableResolver<Parser> {
  const $objectParser =
    objectParser instanceof ZodMiniObject
...
type WithVariableResolver<Parser extends $ZodType> = Parser['_zod']['def']['type'] extends 'string'
  ? ZodMiniPipe<Parser, ZodMiniTransform<unknown, output<Parser>>>
  : ZodSwitch<[[VariableStringParser, DataItemVariableResolverParser], [ZodMiniAny, Parser]]>

export type RecursiveVariableResolver<Parser extends $ZodType> = Parser extends $ZodArray
  ? WithVariableResolver<$ZodArray<RecursiveVariableResolver<Parser['_zod']['def']['element']>>>
  : Parser extends $ZodRecord
...
type ZodObjectConfig<T extends $ZodObject> = T extends $ZodObject<any, infer V> ? V : never
