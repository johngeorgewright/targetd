 ...
 * ```
 */
export function createServer<D extends DT.Any, App extends express.Express = express.Express>(
  data: MaybePromise<D> | (() => MaybePromise<D>),
  { app = express() as App, pathStructure }: CreateServerOptions<D, App> = {},
): App {
  ...
  return server
    .get('/:name', castQueryProp(), castQueryArrayProps(getData), async (req, res) => {
    ...
    .get('/', castQueryProp(), castQueryArrayProps(getData), async (req, res) => {
      res.json(await (await getData()).getPayloadForEachName(res.locals.query ?? req.query))
    })
    .use(errorHandler())
}
